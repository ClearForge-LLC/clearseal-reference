...
| Provenance pack (§1.4) | `npm pack --workspace packages --dry-run` yields `@clearseal/core` only, where `--workspaces` also packed `@clearseal/spike-0100-protocol`. **The PR's provenance dry run agrees:** <https://github.com/ClearForge-LLC/clearseal-reference/actions/runs/36099769790>. `build` ✓, `attest` ✓ (one attestation, for `clearseal-core-0.0.0.tgz`), `release` skipped. The downloaded `release` artifact holds exactly `clearseal-core-0.0.0.tgz` and `provenance.sigstore.json` |
...
| Credentials | none needed. Pushes went over the repository's write deploy key. A short-lived token was minted **only** to open this pull request, kept in a mode-0600 scratch file for that one call, and **deleted** straight after |
