...
1. **Every decision, named and vectored.** The document lists each of the decisions below as a
   ...
   - **A1 — The JSON layer.** *Prior:* **adopt RFC 8785 (JSON Canonicalization Scheme) wholesale**
     ...
     (RFC 8785, the *sorting of object properties* rule), strings escaped per its *serialization of
     strings* rule, numbers per ECMAScript `Number::toString` (its *serialization of numbers* rule),
     UTF-8 output. Reason: a published scheme with independent
