...
| Key-set fetching | **JWKS cached with a TTL; one refetch on an unknown `kid`; a fetch failure with no valid cache refuses, never accepts — answered `503` with `Retry-After`, not `401 invalid_token`, so a correct client keeps its token (amended at `-1003` review; built by `-1003a`); clock-skew tolerance is a stated constant.** | Each of these is a fail-open when left to a library default. |
| Token acceptance | **A token is accepted only when every row of `auth/CHECKS.md` passes, and the keys it may be checked with are themselves checked: an RSA key is used only with an odd exponent of at least 65537 and a modulus of 2048 to 8192 bits; header, payload and key set decode as strict UTF-8.** The principal is `sub`, an opaque string of 1 to 256 characters that handlers treat as opaque; the audit sink renders it escaped. Token lifetime is the issuer's policy, bounded by a stated horizon on `exp` (`-1003a`). `typ` is lenient by default and strict on request (`-1003a`). Trust for a private-CA issuer is scoped to the key-set client, never process-wide. The audience and the resource URL are configured separately — authorization servers that issue an audience identifier rather than a URL are legitimate — and a difference is audited at start. Scopes and `403 insufficient_scope` wait for caller entitlement (P2/P6); until then the metadata says `scopes_supported: []`. | Ruled at `-1003` review and after the external red-team. The key rules came from the red-team's F3: a key set publishing exponent 1 made a signature with no private key behind it verify — reproduced by the architect, whose own review had read the import code and missed it. |
...
| `401` + protected-resource metadata; audience-bound verify | §1, §3, §B, #10 | `auth-rs` | Wrong-audience token → `401`; missing token → `401`; unauthenticated `/health` → `200` | **built** — P1 (`-1003`, `c4ee2f0`): an AS-agnostic verifier on `node:crypto`, every check in `CHECKS.md` with its red-proof; an external red-team found nothing critical or high |
...
| 2026-09-26 | `-1002a` merged (`809a6b9`): a symlink leaf the kernel refuses under `O_NOFOLLOW` is now a recorded, audited containment refusal (the architect's R-1 swap test, re-run, now ends in `ContainmentRefusal`, victim intact); a `read_only` tool's cage opens `r` only; the edition cage seam carries a frozen `CagePolicy`. The cage's docstring now states the limits it cannot close in-process — hard links, a FIFO at the leaf, metadata changes through a read-only descriptor — as the edition OS cage's job. | Both follow-ups from the `-1002` review, closed the same day. |
| 2026-09-26 | `-1003` merged (`c4ee2f0`): the verifier; one §8 row to *built*; §5 gains *Token acceptance*; *Key-set fetching* amended (an outage answers `503`). First WO under the external review cadence: the Adversary red-team ran the real verifier, key-set client and transport against forged tokens and hostile key servers and found nothing critical or high — no dispatch without a valid token, no algorithm or key confusion, no forgery against a correct key set — and six low findings. Three fixed before merge with red-proofs (F1 every refusal audited, contradicting the PR's own `CHECKS.md` until fixed; F3 RSA key parameters — an exponent-1 key allowed forgery, reproduced by the architect; F5 strict UTF-8, lenient decoding merged three subjects into one principal); three deferred to `-1003a` with the outage status, scoped CA trust, an audience-difference audit line and JSON-RPC error audit. Builder decisions ratified: D-1 (the test certificate trusted through the key-set client's own `ca`), D-3 (RFC 6750 §3.1: another scheme `401` with no code, a malformed Bearer `400`). | The architect's review measured the diff and read the key-set client and verifier closely and still missed F3; the outside pass is why it exists. The findings and their dispositions are recorded here and in FEEDBACK under the app identity rather than posted from the red-team's own account. |
| 2026-09-26 | `-1003a` merged (`fcb415f`): an issuer outage answers `503` with `Retry-After` (a refetch that never landed gives its window back — adversarial A1); a lifetime horizon on `exp` (default 24 h); `typ` accepts `JWT`, `at+jwt` and each with the `application/` prefix, with an optional strict `at+jwt` mode — the architect's WO had applied the prefix rule to one type only, fixed before merge; the default audit sink escapes line separators, C1, DEL and Unicode format characters; scoped private-CA trust (`AUTH_JWKS_CA_FILE`, read once without blocking); an audit line when audience and resource URL differ; one `rpc-refused` line per JSON-RPC error. Rulings: A4 accepted as documented (a `503` reveals operational state, not access; claims are never judged before the signature); A7 an empty key set fetched successfully answers `401` (the issuer answered). | All six items the `-1003` review and red-team deferred, closed the same day. |
| 2026-09-26 | `-1004` merged (`bfeb418`): the teaching edition skeleton — `notes.read`, `pins/teaching.json`, a drift test, the supply-boundary test, and `p1-exit.test.ts`, which runs every P1 exit-gate clause against a real teaching node. The builder's first boundary checker passed nine planted editions, five of which did real damage when started; it was rewritten as a strict allowlist evaluated in a child process, and every bypass became a planted case (28 mutants, all red). The checker states that static reading is best-effort: the registry brand and the cage are the run-time boundaries. Builder decisions ratified: D-1 the core as a `peerDependency` (the SBOM check does not follow workspace links; a P2 chore to teach it), D-2 a `.leak-gate-allow` line for the manifests' public digests, D-3 to D-5. Three core findings the builder correctly did not fix inside an edition's WO — admitted tools mutable after admission (A6, an N2 hole reachable from the supply side, not by a model), a FIFO in a root pinning a worker past the timeout (A10), and on `windows-latest` a planted symlink letting `notes.read` read outside its root — go to `-1006`. **P1 does not exit until `-1006` merges.** | The evidence test makes the phase exit a command; the three findings are why the command is not yet enough. |
