...
| Protocol revision | **Serve the newest revision the pinned official SDK offers — measured, currently `2025-11-25` on SDK 1.30.1 — and record the gap to `2026-07-28` as unmet.** The maintenance cadence re-probes on every SDK bump; the ruling is reopened if any consumer requires the newer revision or `server/discover`. *Amended: the genesis ruling targeted `2026-07-28`; the spike showed the SDK does not serve it, and `initialize` never refuses a revision — it counter-offers.* | ClearSeal §B and §9 step 2. The reference node's first spike disproved a written assumption about exactly this — and so did this one. Writing our own transport to reach the newer revision would be the second interpretation of the specification that decision #13 warns against, made by us; serving what the official SDK serves, honestly labelled, is the reference's job until upstream moves. |
